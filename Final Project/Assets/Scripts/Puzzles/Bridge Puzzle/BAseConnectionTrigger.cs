...
//check  if this snaps correctly, might need snap and unsnap methods
public class BAseConnectionTrigger : MonoBehaviour
{
    [SerializeField]
    MeshCollider bridgeBase;
    [SerializeField]
    BoxCollider floor;
    GameObject[] gos;
    ...
    void Start()
    {
        ...
        attachTransform = this.GetComponent<XRSocketInteractor>().attachTransform;
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (!isSnapped)   //if nothing is in snapZone, checks ID for snapping 
        {
            ...
            if (other.gameObject.name.StartsWith("BBridge") && other.gameObject.layer == 8)   //checks for black bridge piece
            {
    ...
    private void OnTriggerExit(Collider other)
    {
        if (!isSnapped)   //if not snapped, reset colots of objects to 
        {
            ...
            if (other.gameObject.name.StartsWith("BBridge"))  //if was valid snap ID, delete reference to snapped item 
            {
